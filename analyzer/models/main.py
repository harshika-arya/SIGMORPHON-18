...
from data_preprocessing.load_data import load_data_for_seq2seq, load_test_data
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, hidden_size, num_layers):
	...
	BidireLSTM_vector= Bidirectional(LSTM(40, dropout=0, return_sequences=True))(hindi_word_embedding)
	'''
...
if MODE == 'train':
	...
	if len(saved_weights) == 0:
		...
		test_sentences = pickle.load(open('./pickle-dumps/sentences_test_20', 'rb'))
		test_roots = pickle.load(open('./pickle-dumps/rootwords_test_20', 'rb'))
		test_features = pickle.load(open('./pickle-dumps/features_test_20', 'rb'))
