...
def write_words_to_file(orig_words, predictions):
	...
	filename = "./outputs/BLSTM_with_context1_and_attention/multitask_context1_out.txt"
...
def write_features_to_file(orig_features, pred_features, encoders):
	...
	for i in range(len(orig_features)):
		filename = "./outputs/BLSTM_with_context1_and_attention/feature"+str(i)+"context1_out.txt"
		with open(filename, 'w', encoding='utf-8') as f:
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6, hidden_size, num_layers, context1=False, context2=False):

	def smart_merge(vectors, **kwargs):
			return vectors[0] if len(vectors)==1 else merge(vectors, **kwargs)		
	
	current_word = Input(shape=(X_max_len,), dtype='int32')
	right_word = Input(shape=(X_max_len,), dtype='int32')
	left_word = Input(shape=(X_max_len,), dtype='int32')
	...
	right_word_embedding = emb_layer(right_word) # these are the left shifted X
	left_word_embedding = emb_layer(left_word) # these are the right shifted X

	BidireLSTM_curr= Bidirectional(LSTM(40, dropout=dropout, return_sequences=True))(current_word_embedding)
	BidireLSTM_right = Bidirectional(LSTM(40, dropout=dropout, return_sequences=True))(right_word_embedding)
	BidireLSTM_left = Bidirectional(LSTM(40, dropout=dropout, return_sequences=True))(left_word_embedding)
	...
	#concatenated_encodings = concatenate([BidireLSTM_curr, BidireLSTM_right, BidireLSTM_left])
	concatenated_encodings = [BidireLSTM_curr]
	concatenated_encodings.append(BidireLSTM_right)
	concatenated_encodings.append(BidireLSTM_left)
	...
	all_inputs = [current_word, right_word, left_word]
...
# X_left & X_right = X shifted to one position left and right for context
X, X_vocab_len, X_word_to_ix, X_ix_to_word, y, y_vocab_len, y_word_to_ix, y_ix_to_word, X_left, X_right = load_data_for_seq2seq(sentences, rootwords, test=False, context1=True)
...
X_left = pad_sequences(X_left, maxlen = X_max_len, dtype='int32', padding='post')
X_right = pad_sequences(X_right, maxlen = X_max_len, dtype='int32', padding='post')
...
model = create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len,
		y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y7, n7, HIDDEN_DIM, LAYER_NUM, context1=True, context2=False)

saved_weights = "./model_weights/multiTask_with_att_and_context1.hdf5"

if MODE == 'train':
	print("Training model ..")
	y_sequences = process_data(y, y_max_len, y_word_to_ix)

	hist = model.fit([X, X_left, X_right], [y_sequences, y1, y2, y3, y4, y5, y7], 
		validation_split=0.25, 
		batch_size=BATCH_SIZE, epochs=EPOCHS,  
		callbacks=[EarlyStopping(patience=7),
		ModelCheckpoint('./model_weights/multiTask_with_att_and_context1.hdf5', save_best_only=True,
			verbose=1)])
	...
	if len(saved_weights) == 0:
		...
		print(test_sentences[:2])
		complete_list,X_test, X_vcab_len, X_wrd_to_ix, X_ix_to_wrd, y_test, y_vcab_len, y_wrd_to_ix, y_ix_to_wrd, X_left, X_right = \
				load_data_for_seq2seq(test_sentences, test_roots, features, labels, test=True, context1=True)
		...
		plot_model(model, to_file="multi_task_attEnc_arch_with_context.png", show_shapes=True)

		print(model.evaluate([X_test,X_left, X_right], [y_test_seq, y1, y2, y3, y4, y5, y7]))
		print(model.metrics_names)
		
		words, f1, f2, f3, f4, f5, f7 = model.predict([X_test, X_left, X_right])
		...
		pickle.dump(pred_features, open('./pickle-dumps/predictions_context1_with_att', 'wb'))
