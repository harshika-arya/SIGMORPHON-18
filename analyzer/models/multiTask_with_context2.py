...
def write_words_to_file(orig_words, predictions):
	...
	filename = "./outputs/BLSTM_context2_and_attention/multitask_context1_out.txt"
...
def write_features_to_file(orig_features, pred_features, encoders):
	...
	for i in range(len(orig_features)):
		filename = "./outputs/BLSTM_context2_and_attention/feature"+str(i)+"context2_out.txt"
		with open(filename, 'w', encoding='utf-8') as f:
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6, hidden_size, num_layers, context1=False, context2=False):
	...
	all_inputs = [current_word, right_word1, right_word2, left_word1, left_word2]
...
X, X_vocab_len, X_word_to_ix, X_ix_to_word, y, y_vocab_len, y_word_to_ix, y_ix_to_word, X_left1, X_left2, X_right1, X_right2 = \
	load_data_for_seq2seq(sentences, rootwords, test=False, context2=True)
...
model = create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len,
		y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y7, n7, HIDDEN_DIM, LAYER_NUM, context1=False, context2=True)
...
if MODE == 'train':
	...
	hist = model.fit([X, X_left1, X_left2, X_right1, X_right2], [y_sequences, y1, y2, y3, y4, y5, y7], 
	...
	if len(saved_weights) == 0:
		...
		complete_list,X_test, X_vcab_len, X_wrd_to_ix, X_ix_to_wrd, y_test, y_vcab_len, y_wrd_to_ix, y_ix_to_wrd, X_left1, X_left2, X_right1, X_right2 = \
				load_data_for_seq2seq(test_sentences, test_roots, features, labels, test=True, context1=False, context2=True)
		...
		print(model.evaluate([X_test, X_left1, X_left2, X_right1, X_right2], [y_test_seq, y1, y2, y3, y4, y5, y7]))
		print(model.metrics_names)
		
		words, f1, f2, f3, f4, f5, f7 = model.predict([X_test, X_left1, X_left2, X_right1, X_right2])
		...
		pickle.dump(pred_features, open('./pickle-dumps/predictions_context2_with_att', 'wb'))
